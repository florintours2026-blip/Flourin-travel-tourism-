...
async function isAdmin(user){if(!user)return false;if(user.uid==='7nE6QoTEPFOk0IhwcZUnymkyzoY2')return true;const s=await getDoc(doc(db,'admins',user.uid));const active=s.exists()?s.data()?.active:false;return active===true||active==='true';}
