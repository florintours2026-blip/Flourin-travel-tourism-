...
async async function isAdmin(user){if(!user)return false;if(user.uid==='7nE6QoTEPFOk0IhwcZUnymkyzoY2')return true;const s=await getDoc(doc(db,'admins',user.uid));const active=s.exists()?s.data()?.active:false;return active===true||active==='true';}
...
async function loadAll(){
 const os=await getDocs(collection(db,'offers')); offers=os.docs.map(d=>({id:d.id,...d.data()}));
...
onAuthStateChanged(auth, async user => {
  try {
    ...
    if ($('adminTheme')) {
      $('adminTheme').textContent = savedTheme === 'dark' ? '☀' : '◐';
      $('adminTheme').setAttribute('aria-label', savedTheme === 'dark' ? 'الوضع الفاتح' : 'الوضع الداكن');
      $('adminTheme').onclick = () => {
        const dark = !document.body.classList.contains('dark');
        document.body.classList.toggle('dark', dark);
        localStorage.setItem('florin-theme', dark ? 'dark' : 'light');
        $('adminTheme').textContent = dark ? '☀' : '◐';
        $('adminTheme').setAttribute('aria-label', dark ? 'الوضع الفاتح' : 'الوضع الداكن');
      };
    }
